...
import styles from "./V5StickyHeader.module.css";
 ...
 */
export type V5NavKind = "anchor" | "page" | "external";
...
function scrollToAnchor(hash: string) {
  ...
  if (!target) {
    console.warn(
      `[V5-StickyHeader] ⚠️ Anchor "${hash}" not found on page — nav link will not scroll. ` +
...
export default function V5StickyHeader({
  alwaysSolid = false,
}: {
  alwaysSolid?: boolean;
} = {}) {
  ...
  useEffect(() => {
    ...
    if (typeof IntersectionObserver === "undefined") {
      console.warn(
        `[V5-StickyHeader] ⚠️ IntersectionObserver unavailable — defaulting to solid header. ` +
    ...
    console.log(`[V5-StickyHeader] 👁️ Sentinel observer attached`);
  ...
  useEffect(() => {
    ...
    function handleKey(e: KeyboardEvent) {
      if (e.key === "Escape") {
        console.log(`[V5-StickyHeader] ⎋ ESC pressed — closing drawer`);
        setMenuOpen(false);
  ...
  const handleNavClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>, link: V5NavLink) => {
      ...
      if (link.kind === "page") {
        ...
        console.log(`[V5-StickyHeader] 🔗 Page nav — push ${link.href}`);
        return;
      }
      ...
      if (router.pathname === "/v5") {
        ...
        console.log(
          `[V5-StickyHeader] 🔗 In-page anchor — scrolling to ${link.href}`
        );
        return;
      }
