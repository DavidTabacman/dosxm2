...
export interface V5ContactoProps {
  id?: string;
...
export default function V5Contacto({
  id = "contacto",
  onSubmit,
  founders,
}: V5ContactoProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!result.ok) {
      console.warn(`[V5-Contacto] ⚠️ Validation failed on field "${result.first}"`);
      if (result.first) {
    ...
    console.log(`[V5-Contacto] 📨 Submitting form — ${JSON.stringify(payload)}`);
    setSubmitting(true);
    try {
      ...
      console.error(`[V5-Contacto] ❌ Submit failed — ${msg}`);
      setSubmitError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  useEffect(() => {
    if (submitted) {
      console.log(`[V5-Contacto] ✅ Form submission complete — success UI visible`);
    }
  }, [submitted]);
